...
add_action('admin_head', array('JDs_WPAlchemy_MetaBox', '_global_head'));

add_action('admin_footer', array('JDs_WPAlchemy_MetaBox', '_global_foot'));
...
class JDs_WPAlchemy_MetaBox
{
	...
	function JDs_WPAlchemy_MetaBox($arr)
	{
	...
	function _init()
	{
		...
		if ( ! JDs_WPAlchemy_MetaBox::_is_post() AND ! JDs_WPAlchemy_MetaBox::_is_page()) return;
	...
	function _setup()
	{
		...
		//include plugins_url()."inc/metaboxes/simple-meta.php";
	...
	{
		if ('post' == JDs_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == JDs_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = JDs_WPAlchemy_MetaBox::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = JDs_WPAlchemy_MetaBox::_get_post_id();
	...
	function _save($post_id) 
	{
		...
		JDs_WPAlchemy_MetaBox::clean($new_data);
	...
	static function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i])) 
				{
					JDs_WPAlchemy_MetaBox::clean($arr[$i]);
